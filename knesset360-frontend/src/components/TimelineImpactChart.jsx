        ...
        return (
            <div className="custom-tooltip bill-tooltip" 
                style={{ 
                    ...
                    padding: '15px', 
                    border: `2px solid ${STATUS_COLORS[data.statusid]}`,
                    borderRadius: '8px',
                ...
                <span style={{ 
                    ...
                    fontSize: '12px', 
                    fontWeight: 'bold',
                    backgroundColor: STATUS_COLORS[data.statusid] || '#9ca3af',
                    color: '#fff',
                    marginBottom: '10px'
                }}>
                    {STATUS_DESC[data.statusid]}
                </span>
                ...
                <div style={{ fontSize: '13px', color: '#4b5563' }}>
                    <div style={{ fontSize: '13px', color: '#4b5563' }}>
                        ...
                        {data.initiators_info?.length < 4 ? (
                            /* Show first 3 in a row */
                            data.initiators_info.map(p => p.name).join(', ')
                        ) : (
                            /* If 4 or more: Show the first 3 + the remaining count */
                            <>
                                {data.initiators_info.slice(0, 3).map(p => p.name).join(', ')}
                                <span style={{ fontWeight: 'bold', color: '#8884d8' }}>
                                    {` +${data.initiators_info.length - 3}`}
                                </span>
                            </>
                        )}
                    </div>
...
export default function TimelineImpactChart({ billsData, scoreData }) {
    ...
    const chartData = useMemo(() => {
        // 1. Process Scores
        const formattedScores = scoreData.map(d => ({
            ...d,
            timestamp: new Date(d.year, d.month - 1, 28).getTime(),
            type: 'score',
        }));

        const formattedBills = billsData.map(bill => {
            const billTs = new Date(bill.publishdate).getTime();

            // Find the two months the bill falls between
            // Doing this to find the f_score_line(bill_date_x) = y_score (vertical line end) with interpolation
            let interpolatedScore = 0;
            const nextScoreIndex = formattedScores.findIndex(s => s.timestamp >= billTs);
            if (nextScoreIndex === -1) 
                ...
                interpolatedScore = prev.score + (billTs - prev.timestamp) * ((next.score - prev.score) / (next.timestamp - prev.timestamp));
            }
            return {
                ...bill,
